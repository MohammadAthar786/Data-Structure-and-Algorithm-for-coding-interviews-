...
**Pattern:** HashMap + Distance Frequency
...
Order matters, so (i, j, k) and (i, k, j) are different.
```
...
```text
Points / coordinates
Same distance from one center point
Need count of ordered tuples
```
...
count how many other points are at each distance.
```
...
There are 2 points at same distance.
...
So for frequency `f` at same distance:
...
For each point i as center:
    Create empty hashmap distanceFreq

    For each point j:
        If i == j, skip

        Calculate squared distance between i and j
        distanceFreq[distance]++

    For each distance frequency f:
        answer += f * (f - 1)
...
```text
No need to use sqrt.
If squared distances are equal, actual distances are equal.
```
...
```text
points = [[0,0]]                 answer = 0
points = [[0,0],[1,1]]           answer = 0
points = [[0,0],[1,0],[-1,0]]    answer = 2
points = [[0,0],[1,0],[2,0]]     answer = 2
```
...
```text
Fix one center point, group other points by distance.
For each group: freq * (freq - 1).
```
